...
- **S9** ✅ Elimination mode (survival gates) — `engine.ts` gate logic (quota crossed → gate closes → stragglers eliminated), editor Gate piece + quota stepper, render gate lines + hide eliminated. Verified: +2 tests (deterministic elimination, no-gate unchanged); editor gate-authoring + Test Race work. (Chose survival-gates over teleport panel-reset.)
- **S10** ✅ Brackets / tournament — `race/brackets.ts` `runBracket()` (groups → top-K advance → final → champion, deterministic, colours tied to original ids), `app/bracketController.ts` + `BracketView.tsx` live match-by-match player, new **Bracket** tab. Verified: +4 orchestration tests; in-browser runBracket produced the expected 5-match plan (16 balls, groups of 4).
- Bundle ≈ 2.5 MB / 871 kB gzip (Rapier base64 wasm ~730 kB gzip + Konva). Code-splitting deferred to S8.
- **Git**: committed on `main` (merged from `stage-6/web-app`) — `9adf97d` (S0–S6), `583058b` (S9), `a717af9` (S10) — and **pushed to origin/main**. The Pages deploy workflow RAN but failed at `actions/configure-pages` because the repo is private (free plan). To deploy: make the repo public (then re-trigger with any push / workflow_dispatch), or host on Vercel/Netlify. Test locally meanwhile: `npm --prefix web run dev` or serve `web/dist`.
- **Browser preview caveat**: the rAF race/bracket loops pause when the tab is hidden (normal browser throttling), so headless screenshots of live motion are unreliable here — races run fine in a real visible browser tab; sim/export/orchestration are all covered by the 27 unit tests regardless.
...
## 🔗 Pointer
→ Current stage folder: `staging/stage-6-web-editor-app/` · **S0–S6 + S9 + S10 built + verified** — all 3 race modes + editor + MP4 export (see Build status above). Only polish remains: **S7** (cross-browser MP4 fallback / labelled WebM), **S8** (perf incl. Rapier code-split off first paint + design pass + QA + silence the harmless Rapier setTranslation deprecation warning). Run `web/`: `npm --prefix web run dev` (or serve `web/dist`); tests `npm --prefix web test` (27 pass). **Deploy is blocked**: repo private → make it public for GitHub Pages, or use Vercel/Netlify. Python engine / Remotion renderer / instagrapi are retired-as-reference.
